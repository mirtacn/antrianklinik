...
@section('title', 'Data Pasien BPJS')
        ...
        <div class="row">
            <div class="card" style="width: 100%; padding: 20px; border: none;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <form class="d-flex">
                        <input class="form-search me-2" type="search" name="search" placeholder="  Cari data pasien ..."
                ...
                <a href="{{ route('tambahpasienbpjs') }}">
                    <button class="btn-tambah">+ Tambah Pasien BPJS</button>
                </a>
                        ...
                        <tbody>
                            @forelse($pasien as $index => $p)
                                <tr>
                                    ...
                                    <td>{{ $p->alamat }}</td>
                                    <td>{{ $p->faskes_tingkat_1 }}</td>
                                    <td>
                                        <a href="{{ route('editpasienbpjs', $p->id) }}" class="btn btn-sm btn-primary me-1">
                                            <i class="bi bi-pencil"></i> Edit
                                        </a>
                                        <form action="{{ route('deletepasienbpjs', $p->id) }}" method="POST" style="display:inline;" id="delete-form-{{ $p->id }}">
                                            @csrf
                                            @method('DELETE')
