...
<body>
    <h2 class="header">Dear {{ $antrian->pasienUmum->nama ?? $antrian->pasienBPJS->nama }},</h2> <!-- Update this line -->
    ...
    <div class="details">
        <p><strong>Layanan:</strong> {{ $antrian->poli->nama_poli }}</p>
        <p><strong>Dokter:</strong> {{ $antrian->jadwaldokter->dokter->nama_dokter ?? '-' }}</p>
        <p><strong>Tanggal Antrian:</strong> {{ \Carbon\Carbon::parse($antrian->tanggal_antrian)->translatedFormat('d F Y') }}</p>
        <p><strong>Estimasi Dilayani:</strong> {{ \Carbon\Carbon::parse($antrian->waktu_estimasi)->format('H:i') }}</p>
        <p><strong>Nomor Antrian:</strong> {{ $antrian->poli->kode_poli }}{{ $antrian->no_antrian }}</p>
    </div>
